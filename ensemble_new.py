...
# ─────────────────────────────────────────────────────────────────────────────
def ensemble_predict(cnn_model, rf_model, X_cnn, X_rf, alpha=0.5):
    """
    alpha  = weight for CNN (1-alpha for RF).
    Default: equal weighting (0.5 / 0.5).
    """
    p_cnn = cnn_model.predict(X_cnn, verbose=0)          # (N, 3)
    p_rf  = rf_model.predict_proba(X_rf)                 # (N, 3)
    p_ens = alpha * p_cnn + (1 - alpha) * p_rf           # (N, 3)
    return p_ens
...
#  4.  Plots
# ─────────────────────────────────────────────────────────────────────────────
def plot_confusion_matrix(y_true, y_pred, out_path="ensemble_confusion_matrix_new.png"):
...
def plot_roc_curve(y_true, y_proba, out_path="ensemble_roc_curve_new.png"):
...
def main():
    ...
    # ── ensemble predict ──────────────────────────────────────────────────────
    print("\n🤝 Running Soft-Voting Ensemble (α=0.5) …")
    p_ens  = ensemble_predict(cnn_model, rf_model, X_te_cnn, X_te_rf, alpha=0.5)
