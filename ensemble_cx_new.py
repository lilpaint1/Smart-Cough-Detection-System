...
# ─────────────────────────────────────────────────────────────────────────────
def ensemble_predict(cnn_model, xgb_model, X_cnn, X_rf):
    """
    Soft-voting 2 ทาง — เฉลี่ยเท่ากัน (CNN + XGB)
    XGB ใช้ฟีเจอร์ 416-D (X_rf), CNN ใช้ mel-spectrogram
    """
    p_cnn = cnn_model.predict(X_cnn, verbose=0)          # (N, 3)
    p_xgb = xgb_model.predict_proba(X_rf)                # (N, 3)
    p_ens = (p_cnn + p_xgb) / 2.0                        # (N, 3)
    return p_ens
...
#  4.  Plots
# ─────────────────────────────────────────────────────────────────────────────
def plot_confusion_matrix(y_true, y_pred, out_path="ensemble_cx_confusion_matrix_new.png"):
    cm = confusion_matrix(y_true, y_pred)
    plt.figure(figsize=(7, 6))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                xticklabels=CLASSES, yticklabels=CLASSES)
    plt.title("Confusion Matrix – Ensemble (CNN + XGB)")
...
def plot_roc_curve(y_true, y_proba, out_path="ensemble_cx_roc_curve_new.png"):
    ...
    plt.title("ROC Curve – Ensemble (CNN + XGB)")
...
def main():
    ...
    # ── ensemble predict ──────────────────────────────────────────────────────
    print("\n🤝 Running Soft-Voting Ensemble 2-way (CNN + XGB, เฉลี่ยเท่ากัน) …")
    p_ens  = ensemble_predict(cnn_model, xgb_model, X_te_cnn, X_te_rf)
    y_pred = np.argmax(p_ens, axis=1)

    # ── report ────────────────────────────────────────────────────────────────
    print("\n--- Classification Report (Ensemble CNN+XGB) ---")
    ...
    print("\n🎉 Done!  Output files:")
    print("   ensemble_cx_confusion_matrix_new.png")
    print("   ensemble_cx_roc_curve_new.png")
